...
class QwenAdapter(BaseLLMAdapter):
    ...
    async def upload_attachments(self, request: ChatRequest) -> "QwenAdapter":
        ...
        if other_paths:
            ...
            await file_input.set_input_files(other_paths)
            await asyncio.sleep(1.0)
    ...
    async def send(self) -> "QwenAdapter":
        ...
        await self.page.locator(SELECTORS["send_btn"]).click()
        return self
    ...
    async def select_model(self, model):
        ...
        model_item = self.page.locator(SELECTORS["model_menu"]).filter(has_text=model).first
